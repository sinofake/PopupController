...
@property (nonatomic, strong) UIView *actionSheetStyleBgView;
...
- (void)orientationWillChange {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        self.maskView.frame = self.holderView.bounds;
...
- (void)presentPopupControllerAnimated:(BOOL)flag {
    [self presentPopupControllerInView:[[UIApplication sharedApplication] keyWindow] animated:flag];
}
...
- (void)presentPopupControllerInView:(UIView *)holderView animated:(BOOL)flag {
    ...
    self.maskView.frame = holderView.bounds;
    ...
    [self.holderView addSubview:self.maskView];
    
    if (self.theme.popupStyle == CNPPopupStyleActionSheet) {
        self.actionSheetStyleBgView = [[UIView alloc] initWithFrame:self.holderView.bounds];
        self.actionSheetStyleBgView.backgroundColor = self.maskView.backgroundColor;
        self.maskView.backgroundColor = [UIColor clearColor];
        [self.holderView insertSubview:self.actionSheetStyleBgView belowSubview:self.maskView];
        
        self.actionSheetStyleBgView.alpha = 0;
        [UIView animateWithDuration:flag?0.3:0.0 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
            self.actionSheetStyleBgView.alpha = 1.f;
            self.popupView.center = [self endingPoint];
...
- (void)dismissPopupControllerAnimated:(BOOL)flag {
    ...
    if (self.theme.popupStyle == CNPPopupStyleActionSheet) {
        [UIView animateWithDuration:flag?0.3:0.0 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
            ...
            [self.actionSheetStyleBgView removeFromSuperview];
            self.actionSheetStyleBgView = nil;
...
- (CGPoint)dismissedPoint {
    ...
    switch (self.theme.presentationStyle) {
        ...
        case CNPPopupPresentationStyleSlideInFromBottom:
            ...
            if (self.theme.popupStyle == CNPPopupStyleActionSheet) {
                dismissed = CGPointMake(self.maskView.center.x, self.maskView.bounds.size.height + self.popupView.bounds.size.height * 0.5f);
            }
            break;
        case CNPPopupPresentationStyleSlideInFromLeft:
